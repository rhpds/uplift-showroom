...
. To find the Kiali web ui, select it from the menu on the upper right of the screen.
...
+
. Log in to the Kiali web interface with your username `{user}` and password `{password}`.
